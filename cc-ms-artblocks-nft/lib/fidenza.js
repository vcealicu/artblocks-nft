...
const rnd = () => {
    ...
    nScts = 10;
let moduleExports = {};
moduleExports.sketch = function(p, preloaded) {
    let canvas, ww, wh, wr, LX, RX, TY, BY, spc, z0, z1, z2, z3, z4, z5, z6, z7, sW, sH;
    const dw = 2e3,
        dh = 2400,
        V1 = 1,
        V2 = 2,
    ...
    p.setup = () => {
        preloaded.windowHeight >= 1.2 * preloaded.windowWidth ? (ww = preloaded.windowWidth, wh = 1.2 * preloaded.windowWidth) : (wh = preloaded.windowHeight, ww = preloaded.windowHeight / 1.2);
        wr = ww / dw;
        canvas = p.createCanvas(ww, wh);
        p.colorMode(p.HSB, 360, 100, 100, 100);
        set_seed(preloaded.seed);
        p.randomSeed(0);
        p.noiseSeed(0);
        LX = -500;
        RX = 2500;
    ...
    function flwP(a, b, c) {
        ...
        for (let f = LX; f < RX; f += spc) {
            const b = [];
            for (let e, g = TY; g < BY; g += spc) e = a, c && (e = angle(f, g, w(.5), h(.4)) - pi(.5), d = p.dist(f, g, w(.5), h(.5)), e += rscl(d, 0, w(1.5), 0, pi(1))), b.push(e);
            e.push(b);
        }
        let f = 0,
            g = 0;
    ...
    function flwL(a, b, c, e, f) {
        ...
        for (let k = 0; k < b.length; k++) {
            ...
            for (let b = 0; b < m; b++) {
                ...
                let j = e;
                0 <= c && c < h && 0 <= b && b < g && (j = a[b][c]), f && (j = snp(j, pi(.2))), n += i * p.cos(j), o += i * p.sin(j);
            }
            j.push(l);
        }
        return j;
    ...
    function cllsn(a, b, c, e, f, g) {
        if (g && p.dist(a, b, w(.5), h(.4)) <= 1.3 * c) return !0;
        const j = sctrs(a, b, c);
        for (let h = 0; h < j.length; h++) {
            ...
            for (const g of k) {
                const [e, h, i, j] = g;
                if (p.dist(a, b, e, h) <= c + i && f !== j) return !0;
            }
        }
        return !1;
    }
    ...
    function lerpCrv(a, b, c) {
        ...
        for (let e = 1; e < a.length; e++) {
            const [b, c] = a[e - 1], [f, i] = a[e], j = p.dist(b, c, f, i), k = h + j;
            if (k > g) {
                const a = g - h,
                    e = a / j,
                    k = lrp(b, f, e),
    ...
    function strokeSegment(a, b, c, e) {
        p.stroke(a[0], a[1], a[2]), p.noFill(), swght(w(.001));
        const f = b / w(4e-4),
            g = rng(0, 1e4);
        for (let h, i = 0; i < f; i += 1) {
            ...
            for (let a = b; a < j; a += .01) {
                let b = p.noise(4 * (a * (c / w(.25))) + g, 1.5 * h),
                    f = h + .15 * (.5 - b);
                const [i, j] = e(a, f);
                vrtx(i, j);
            }
    ...
    function fSeg(a, b, c, e, f, g, h, i) {
        p.fill(a[0], a[1], a[2]), b ? (p.stroke(0, 0, 10), swght(w(.001))) : (p.stroke(a[0], a[1], a[2]), swght(w(5e-4)));
        const j = [],
            k = [];
        for (let l = f; l < g; l += .01) j.push(l), k.unshift(l);
        j.push(g), p.beginShape();
        for (const k of j) {
    ...
    p.draw = () => {
        ...
        const r = od(.025) && "wM" !== d && "blk" !== d && "dL" !== d && "pG" !== d && "wOC" !== d,
            ...
            t = !p2 && ("blk" === d || "wOC" === d || od(.2) && 70 < j[2] && a >= V5 && "pG" !== d),
            u = od(.04),
            v = flwP(c, b, o),
        ...
        for (const b of x) {
            ...
            if (2 <= b.points.length) {
                ...
                p.fill(f[0], f[1], f[2]), t ? (p.stroke(0, 0, 15), swght(w(.001))) : p.noStroke();
                let c = z4;
                ...
                const g = fatTop(b.points, c),
                    ...
                    l = crvL(g),
                    m = crvL(i);
                if (p2) strokeSegment(f, c, l, function(a, b) {
        ...
        preloaded.canvasBuffer(Buffer.from(canvas.canvas.toDataURL().replace(/^data:image\/png;base64,/, ""), "base64"));
    };
};
